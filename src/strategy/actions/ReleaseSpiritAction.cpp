...
bool ReleaseSpiritAction::Execute([[maybe_unused]] Event event)
{
    if (bot->IsAlive())
    {
...
bool AutoReleaseSpiritAction::Execute([[maybe_unused]] Event event)
{
    // Death Count to prevent skeleton piles
    Player* master = GetMaster();
    if (!master || (master && GET_PLAYERBOT_AI(master)))
...
bool RepopAction::Execute([[maybe_unused]] Event event)
{
