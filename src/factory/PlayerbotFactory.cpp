    ...
    if (order.empty())
    {
        ...
        for (auto i = spells.begin(); i != spells.end(); ++i, ++row)
        {
                   ...
                   attemptCount++ < 10 && pet->GetFreeTalentPoints())
            {
                ...
                int maxRank = 0;
                for (uint32 rank = 0; rank < std::min<uint32>(MAX_TALENT_RANK, pet->GetFreeTalentPoints()); ++rank)
                {
...
void PlayerbotFactory::ApplyEnchantAndGemsNew([[maybe_unused]] bool destoryOld)
{
